...
- [Avviso della metrica](/azure/azure-monitor/platform/alerts-metric). Avvisi sui dati nel database di metriche di monitoraggio di Azure. Gli avvisi si verificano quando un valore monitorato supera una soglia definita dall'utente e quindi nuovamente quando torna allo stato "normale".

- [Avviso di query di log](/azure/azure-monitor/platform/alerts-log-query). Disponibile per gli avvisi sul contenuto nell'Application Insights o negli archivi di log di Azure. Può anche inviare un avviso in base alle query tra aree di lavoro.

- [Avviso del log attività](/azure/azure-monitor/platform/alerts-activity-log). Avvisi sugli elementi nell'archivio dei log attività, ad eccezione dei dati di integrità del servizio.

- [Avviso di integrità del servizio](/azure/azure-monitor/platform/alerts-activity-log-service-notifications?toc=%2fazure%2fservice-health%2ftoc.json). Un tipo speciale di avviso, solo per i problemi di integrità del servizio provenienti dall'archivio dei log attività.
...
- [Soglie dinamiche](/azure/azure-monitor/platform/alerts-dynamic-thresholds). Le soglie dinamiche esaminano l'attività della risorsa in un periodo di tempo e creano soglie di "comportamento normale" superiori e inferiori. Quando la metrica monitorata non rientra in queste soglie, viene generato un avviso.
...
Assicurarsi di prendere nota delle [limitazioni](/azure/azure-subscription-service-limits#azure-monitor-limits) del numero di avvisi che è possibile creare. Alcuni limiti (ma non tutti) possono essere aumentati chiamando il supporto.
