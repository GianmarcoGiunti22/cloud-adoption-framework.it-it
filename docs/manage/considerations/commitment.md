...
Le organizzazioni possono anche scegliere di [esternalizzare alcune delle funzioni di gestione in corso a un provider di servizi](https://www.microsoft.com/cloud-adoption-framework-offers?ot=manage). Questi provider di servizi possono usare [Azure Lighthouse](https://azure.com/lighthouse) per offrire alle organizzazioni maggiore precisione e controllo nella concessione dell'accesso alle risorse, oltre a una maggiore trasparenza nelle azioni eseguite dai provider di servizi.
...
Per la maggior parte delle aziende, la gestione è più semplice quando tutti gli asset si trovano in un singolo tenant. Tuttavia, alcune organizzazioni potrebbero dover gestire più tenant. L'articolo sulla [centralizzazione delle operazioni di gestione con Azure Lighthouse](../centralize-operations.md) offre alcuni esempi in cui le aziende possono richiedere ambienti Azure multi-tenant.
